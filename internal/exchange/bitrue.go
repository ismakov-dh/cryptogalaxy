...
func (e *bitrue) cfgLookup(markets []config.Market) error {
...
func (e *bitrue) connectWs(ctx context.Context) error {
	ws, err := connector.NewWebsocket(ctx, &e.connCfg.WS, config.BitrueWebsocketURL)
	if err != nil {
...
func (e *bitrue) closeWsConnOnError(ctx context.Context) error {
	<-ctx.Done()
	err := e.ws.Conn.Close()
	if err != nil {
		return err
...
func (e *bitrue) subWsChannel(market string, channel string) error {
	if channel == "trade" {
...
func (e *bitrue) readWs(ctx context.Context) error {
...
func (e *bitrue) processWs(ctx context.Context, wr *wsRespBitrue, cd *commitData, itv *influxTimeVal) error {
	switch wr.Channel {
	case "ticker":
...
// buffers the same in memory and
// then sends it to different storage systems for commit through go channels.
func (e *bitrue) processREST(ctx context.Context, mktID string, mktCommitName string, channel string, interval int) error {
	var (
		req *http.Request
		q   url.Values
		err error
	...
	switch channel {
	case "ticker":
		req, err = e.rest.Request(ctx, "GET", config.BitrueRESTBaseURL+"ticker/price")
		if err != nil {
			if !errors.Is(err, ctx.Err()) {
				logErrStack(err)
			}
		...
		q.Add("symbol", mktID)
	case "trade":
		req, err = e.rest.Request(ctx, "GET", config.BitrueRESTBaseURL+"trades")
		if err != nil {
			if !errors.Is(err, ctx.Err()) {
				logErrStack(err)
			}
