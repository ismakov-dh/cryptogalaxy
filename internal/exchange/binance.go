...
func NewBinance(appCtx context.Context, markets []config.Market, connCfg *config.Connection) error {
	...
	for _, market := range markets {
		for _, info := range market.Info {
			...
			case "rest":
				if restCount == 0 {
					client, err := connectRest("binance")
					if err != nil {
						return err
					}
					e.rest = client
...
func (e *binance) cfgLookup(markets []config.Market) error {
	var id int
...
func (e *binance) connectWs(ctx context.Context) error {
	ws, err := connector.NewWebsocket(ctx, &e.connCfg.WS, config.BinanceWebsocketURL)
	if err != nil {
...
func (e *binance) closeWsConnOnError(ctx context.Context) error {
	<-ctx.Done()
	err := e.ws.Conn.Close()
	if err != nil {
		return err
...
func (e *binance) subWsChannel(market string, channel string, id int) error {
	if channel == "ticker" {
		channel = "miniTicker"
	}
...
func (e *binance) readWs(ctx context.Context) error {
	...
	for {
		...
		default:
			...
			if wr.ID != 0 {
				log.Debug().Str("exchange", "binance").Str("func", "readWs").Str("market", e.channelIds[wr.ID][0]).Str("channel", e.channelIds[wr.ID][1]).Msg("channel subscribed")
				continue
			}
			if wr.Msg != "" {
...
func (e *binance) processWs(ctx context.Context, wr *wsRespBinance, cd *commitData, itv *influxTimeVal) error {
	switch wr.Event {
	case "ticker":
...
// buffers the same in memory and
// then sends it to different storage systems for commit through go channels.
func (e *binance) processREST(ctx context.Context, mktID string, mktCommitName string, channel string, interval int) error {
	var (
		req *http.Request
		q   url.Values
		err error
	...
	switch channel {
	case "ticker":
		req, err = e.rest.Request(ctx, "GET", config.BinanceRESTBaseURL+"ticker/price")
		if err != nil {
			if !errors.Is(err, ctx.Err()) {
				logErrStack(err)
			}
		...
		q.Add("symbol", mktID)
	case "trade":
		req, err = e.rest.Request(ctx, "GET", config.BinanceRESTBaseURL+"trades")
		if err != nil {
			if !errors.Is(err, ctx.Err()) {
				logErrStack(err)
			}
