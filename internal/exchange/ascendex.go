	...
	for {
		...
		if err != nil {
			...
			if retryCount > retry.Number {
				return fmt.Errorf("not able to connect ascendex exchange even after %v retry. please check the log for details", retry.Number)
			}

			log.Error().Str("exchange", "ascendex").Int("retry", retryCount).Msg(fmt.Sprintf("retrying functions in %v seconds", retry.GapSec))
...
func (a *ascendex) processWs(ctx context.Context, wr *wsRespAscendex, cd *commitData) error {
	switch wr.Type {
	case "ticker":
