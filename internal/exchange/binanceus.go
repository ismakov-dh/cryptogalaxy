...
func NewBinanceUS(appCtx context.Context, markets []config.Market, connCfg *config.Connection) error {
	...
	for _, market := range markets {
		for _, info := range market.Info {
			...
			case "rest":
				if restCount == 0 {
					client, err := connectRest("binance-us")
					if err != nil {
						return err
					}
					e.rest = client
...
func (e *binanceUS) cfgLookup(markets []config.Market) error {
	var id int
...
func (e *binanceUS) connectWs(ctx context.Context) error {
	ws, err := connector.NewWebsocket(ctx, &e.connCfg.WS, config.BinanceUSWebsocketURL)
	if err != nil {
...
func (e *binanceUS) closeWsConnOnError(ctx context.Context) error {
	<-ctx.Done()
	err := e.ws.Conn.Close()
	if err != nil {
		return err
...
func (e *binanceUS) subWsChannel(market string, channel string, id int) error {
	if channel == "ticker" {
		channel = "miniTicker"
	}
...
func (e *binanceUS) readWs(ctx context.Context) error {
	...
	for {
		...
		default:
			...
			if wr.ID != 0 {
				log.Debug().Str("exchange", "binance-us").Str("func", "readWs").Str("market", e.channelIds[wr.ID][0]).Str("channel", e.channelIds[wr.ID][1]).Msg("channel subscribed")
				continue
			}
			if wr.Msg != "" {
...
func (e *binanceUS) processWs(ctx context.Context, wr *wsRespBinanceUS, cd *commitData, itv *influxTimeVal) error {
	switch wr.Event {
	case "ticker":
...
// buffers the same in memory and
// then sends it to different storage systems for commit through go channels.
func (e *binanceUS) processREST(ctx context.Context, mktID string, mktCommitName string, channel string, interval int) error {
	var (
		req *http.Request
		q   url.Values
		err error
	...
	switch channel {
	case "ticker":
		req, err = e.rest.Request(ctx, "GET", config.BinanceUSRESTBaseURL+"ticker/price")
		if err != nil {
			if !errors.Is(err, ctx.Err()) {
				logErrStack(err)
			}
		...
		q.Add("symbol", mktID)
	case "trade":
		req, err = e.rest.Request(ctx, "GET", config.BinanceUSRESTBaseURL+"trades")
		if err != nil {
			if !errors.Is(err, ctx.Err()) {
				logErrStack(err)
			}
