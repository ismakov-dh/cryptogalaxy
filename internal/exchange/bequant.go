...
func NewBequant(appCtx context.Context, markets []config.Market, connCfg *config.Connection) error {
	...
	for _, market := range markets {
		for _, info := range market.Info {
			...
			case "rest":
				if restCount == 0 {
					client, err := connectRest("bequant")
					if err != nil {
						return err
					}
					e.rest = client
...
func (e *bequant) cfgLookup(markets []config.Market) error {
	var id int
...
func (e *bequant) connectWs(ctx context.Context) error {
	ws, err := connector.NewWebsocket(ctx, &e.connCfg.WS, config.BequantWebsocketURL)
	if err != nil {
...
func (e *bequant) closeWsConnOnError(ctx context.Context) error {
	<-ctx.Done()
	err := e.ws.Conn.Close()
	if err != nil {
		return err
...
func (e *bequant) subWsChannel(market string, channel string, id int) error {
	if channel == "ticker" {
...
func (e *bequant) readWs(ctx context.Context) error {
		...
		default:
			...
			if wr.Result.Channel != "" {
				if wr.Result.Channel == "ticker/price/1s" {
					log.Debug().Str("exchange", "bequant").Str("func", "readWs").Str("market", e.channelIds[wr.ID][0]).Str("channel", "ticker").Msg("channel subscribed")
				} else {
					log.Debug().Str("exchange", "bequant").Str("func", "readWs").Str("market", e.channelIds[wr.ID][0]).Str("channel", "trade").Msg("channel subscribed")
				}
				continue
			}
...
func (e *bequant) processWs(ctx context.Context, wr *wsRespBequant, cd *commitData, itv *influxTimeVal) error {
	switch wr.Channel {
	case "ticker":
...
// buffers the same in memory and
// then sends it to different storage systems for commit through go channels.
func (e *bequant) processREST(ctx context.Context, mktID string, mktCommitName string, channel string, interval int) error {
	var (
		req *http.Request
		q   url.Values
		err error
	...
	switch channel {
	case "ticker":
		req, err = e.rest.Request(ctx, "GET", config.BequantRESTBaseURL+"ticker")
		if err != nil {
			if !errors.Is(err, ctx.Err()) {
				logErrStack(err)
			}
		...
		q.Add("symbols", mktID)
	case "trade":
		req, err = e.rest.Request(ctx, "GET", config.BequantRESTBaseURL+"trades")
		if err != nil {
			if !errors.Is(err, ctx.Err()) {
				logErrStack(err)
			}
