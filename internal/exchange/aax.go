...
func (e *aax) buildRestRequest(ctx context.Context, mktID string, channel string) (req *http.Request, err error) {
	...
	switch channel {
	case "ticker":
		req, err = e.wrapper.rest.Request(ctx, "GET", e.wrapper.config.RestUrl+"market/candles")
		if err != nil {
		...
		q.Add("timeFrame", "1m")
	case "trade":
		req, err = e.wrapper.rest.Request(ctx, "GET", e.wrapper.config.RestUrl+"market/trades")
		if err != nil {
			if !errors.Is(err, ctx.Err()) {
				logErrStack(err)
			}
