...
func (e *ftx) cfgLookup(markets []config.Market) error {
...
func (e *ftx) connectWs(ctx context.Context) error {
	ws, err := connector.NewWebsocket(ctx, &e.connCfg.WS, config.FtxWebsocketURL)
	if err != nil {
...
func (e *ftx) closeWsConnOnError(ctx context.Context) error {
	<-ctx.Done()
	err := e.ws.Conn.Close()
	if err != nil {
		return err
...
func (e *ftx) subWsChannel(market string, channel string) error {
	if channel == "trade" {
		channel = "trades"
	}
...
func (e *ftx) readWs(ctx context.Context) error {
...
func (e *ftx) processWs(ctx context.Context, wr *wsRespFtx, cd *commitData, itv *influxTimeVal) error {
	switch wr.Channel {
	case "ticker":
...
// buffers the same in memory and
// then sends it to different storage systems for commit through go channels.
func (e *ftx) processREST(ctx context.Context, mktID string, mktCommitName string, channel string, interval int) error {
	var (
		req *http.Request
		q   url.Values
		err error
	...
	switch channel {
	case "ticker":
		req, err = e.rest.Request(ctx, "GET", config.FtxRESTBaseURL+"markets/"+mktID)
		if err != nil {
			if !errors.Is(err, ctx.Err()) {
				logErrStack(err)
			}
	...
	case "trade":
		req, err = e.rest.Request(ctx, "GET", config.FtxRESTBaseURL+"markets/"+mktID+"/trades")
		if err != nil {
			if !errors.Is(err, ctx.Err()) {
				logErrStack(err)
			}
